...
export default function hermesCPlugin({
	...
	return {
		...
		generateBundle(_, bundle) {
			...
			const file = Object.values(bundle)[0] as OutputChunk
			...
			if (cmd.exitCode) {
				if (cmd.stderr.length)
					throw new Error(`Got error from hermesc: ${cmd.stderr.toString()}`)
				else throw new Error(`hermesc exited with code: ${cmd.exitCode}`)
			}
